#!/usr/bin/env python3
"""
CA-CHARQ v5 (MAX) — Multi-Chain Concurrent Cooperative HARQ with MAC
=====================================================================
3 independent chains share 6 free-roaming helpers (CA-CHARQ).
Cross-chain scheduler with priority queue, half-duplex constraint,
receiver single-signal constraint, and collision modelling.
...
class UnderwaterNode:
    ...
    def contend(self, pkt):
        ...
        if cancel_ev not in result:
            ...
            self.helper_tx_cnt[pid] += 1
            nack_cid = getattr(pkt, 'chain_id', 0)
            # v5: route through scheduler for serialized helper access
            if self.scheduler:
                status, rv_s = self.scheduler.request(
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"  CA-CHARQ v5 MAX — Multi-Chain + MAC | {N_RUNS} MC × {len(SNR_LIST)} SNR × {len(PROTOCOLS)} proto")
    ...
    ax1.set_title("CA-CHARQ v5 MAX — 3-Chain with MAC")
    ...
    plt.tight_layout()
    plt.savefig("output/max-v5/max-v5_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/max-v5/max-v5_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/max-v5/max-v5_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/max-v5/max-v5_EE.png")
    plt.close('all')

    print("=" * 60)
    print("CA-CHARQ v5 MAX with MAC Done.")
