...
def per_rs_vec(ser_vec, t_error, n_mc=30):
    """
...
def per_rs_rs_ser(avg_rs_ser, n_sym, t_error):
    """
    PER from RS-symbol-level average SER.
    Caller must correctly group QPSK positions → RS symbols, then average RS SER.
    Bypasses _qpsk_to_rs_ser conversion — input is already RS-level.
    """
    p_s_prime = rs_decoded_ser(avg_rs_ser, n_sym, t_error)
    return float(np.clip(1.0 - (1.0 - p_s_prime) ** n_sym, 1e-15, 1.0 - 1e-15))
...
if __name__ == "__main__":
    ...
    plot_results(SNR_WIDE, r_wide, suffix="v2_full", out_dir="output/pro-v2")
