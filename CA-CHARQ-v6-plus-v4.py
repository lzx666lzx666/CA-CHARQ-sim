...
def compute_packet_per(soft_buffer):
    acc_mi = np.sum(np.log2(1.0 + np.clip(soft_buffer, 0, None))) * BITS_PER_CHUNK
    mi_ratio = acc_mi / TARGET_MI
    per = 1.0 / (1.0 + math.exp(MOD_K * (mi_ratio - 1.0)))
    ...
    def handle(self, pkt):
        ...
        if pkt.pkt_type == PKT_DATA:
            ...
            # Helper 侦听源端数据
                ...
                if random.random() > per:
                    if self.protocol == PROTO_CA:
                        acc_mi_h = np.sum(np.log2(1.0 + np.clip(self.soft_buffer[pid], 0, None))) * BITS_PER_CHUNK
                        c, cpkt = confidence_quantize(acc_mi_h)
...
if __name__ == "__main__":
    ...
    print(f" PHY: MI+sigmoid PER (K={MOD_K}, TARGET_MI={TARGET_MI})")
    print("=" * 65)
    ...
    plt.tight_layout()
    plt.savefig("v6-plus-v4_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] v6-plus-v4_Delay_Overhead.png")
    plt.close('all')
