...
# ==========================================
# 10. W1/W3 Sensitivity (replaces main)
# ==========================================
if __name__ == "__main__":
    import itertools
    W1_LIST = [0.20, 0.30, 0.40, 0.50]
    W3_LIST = [0.20, 0.35, 0.50]
    FIXED_SNR = 0.0
    SIM_T = 3000
    N_RUNS = 2

    heatmap = np.zeros((len(W1_LIST), len(W3_LIST)))

    print("=" * 65)
    print(f" W1/W3 Sensitivity Sweep (CA-CHARQ only), SNR={FIXED_SNR}dB")
    print(f" W1={W1_LIST}, W3={W3_LIST}, SimTime={SIM_T}s")
    print("=" * 65)

    for i, w1 in enumerate(W1_LIST):
        for j, w3 in enumerate(W3_LIST):
            w2 = 1.0 - w1 - w3
            if w2 < 0.05:
                heatmap[i][j] = np.nan
                print(f"  W1={w1:.2f}, W3={w3:.2f} → W2={w2:.2f} SKIP (W2<0.05)")
                continue
            W1 = w1; W2 = w2; W3 = w3
            r = mc_run(FIXED_SNR, PROTO_CA, SIM_T, N_RUNS)
            heatmap[i][j] = r['delay_mean']
            print(f"  W1={w1:.2f}, W3={w3:.2f}, W2={w2:.2f} → D={r['delay_mean']:8.1f}s")

    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    im = ax.imshow(heatmap, cmap='RdYlGn_r', aspect='auto', origin='lower',
                    extent=[W3_LIST[0]-0.075, W3_LIST[-1]+0.075,
                            W1_LIST[0]-0.05, W1_LIST[-1]+0.05])
    for i in range(len(W1_LIST)):
        for j in range(len(W3_LIST)):
            if not np.isnan(heatmap[i,j]):
                ax.text(W3_LIST[j], W1_LIST[i], f'{heatmap[i,j]:.0f}',
                        ha='center', va='center', fontsize=9)
    ax.set_xlabel('W3 (propagation weight)')
    ax.set_ylabel('W1 (confidence weight)')
    ax.set_title('CA-CHARQ Delay (s) vs Competition Weights (SNR=0dB)')
    plt.colorbar(im, ax=ax, label='Delay (s)')
    plt.tight_layout()
    plt.savefig("output/AppendixA_weights.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/AppendixA_weights.png")
    plt.close('all')
    ...
    print(f" PHY: MIESM (3GPP NR) — Cpkt also MIESM-based, no Shannon MI")
    print("=" * 65)
    ...
    plt.tight_layout()
    plt.savefig("output/v11_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/v11_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/v11_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/v11_EE.png")
    plt.close('all')
