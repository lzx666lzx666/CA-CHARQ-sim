...
class UnderwaterNode:
    ...
    def tx_loop(self):
        while True:
            ...
            # S&W / C-ARQ: NACK-back triggers immediate retry → short timeout
            # C-HARQ: wait for helper FEC (Pac-1+Pac-2 ~0.14s TX + prop)
            # CA-CHARQ: similar to C-HARQ but Grace-period aware
            if self.protocol == PROTO_CHARQ:
                helper_tx = (CHARQ_FEC_EXTRA * RS_BITS_PER_SYM) / BIT_RATE + T_PROTECTION_GAP
                gto = rtt + helper_tx + HOP_DIST / SOUND_SPEED + 1.0
            ...
            for retry_i in range(MAX_RETRIES):
                ...
                if pid in self.pending_response:
                    ...
                    if msg['type'] == 'ACK':
                        ...
                        pass  # C-HARQ: don't retransmit, wait for helper via timeout
                    else:
                        yield self.env.process(self._send_data(
                            self.next_hop_id, pid, 0, creation_time))
                        continue
                ...
                if ack_ev in result:
                    ...
                    if msg['type'] == 'ACK':
                        ...
                        if self.protocol == PROTO_CA:
                            ...
                            # S&W / C-ARQ: retransmit immediately on NACK
                            yield self.env.process(self._send_data(
                                self.next_hop_id, pid, 0, creation_time))
                        # C-HARQ: don't retransmit on NACK; let timeout trigger retry
                else:
                    yield self.env.process(self._send_data(
                        self.next_hop_id, pid, 0, creation_time))
    ...
    def _compute_per(self, soft_buf):
        """PER: per-symbol product for no-FEC; per RS-symbol group for FEC."""
        ...
        # RS FEC: group every 3 QPSK positions → 1 RS symbol SER via product
        ...
        return per_rs_vec(ser_rs_vec, t_eff)
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig("output/simpy_v2_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/simpy_v2_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/simpy_v2_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/simpy_v2_EE.png")
    plt.close('all')
