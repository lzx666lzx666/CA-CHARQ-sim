...
def per_rs_vec(ser_vec, t_error, n_mc=80):
    """
    Accurate RS PER from per-symbol RS SER vector (heterogeneous SNRs).
    ser_vec[i] = RS-symbol error prob; packet fails if > t_error errors.
    Monte Carlo over Poisson-binomial — matches per-symbol fading diversity.
    """
    n_sym = len(ser_vec)
    if n_sym == 0:
        return 1.0
    errors = np.sum(np.random.rand(n_mc, n_sym) < np.array(ser_vec), axis=1)
    return float(np.mean(errors > t_error))
...
def per_rs_expected_one_path(k, gamma_lin, n_sym=RS_N_SYM, t_error=RS_T_BASE, n_mc=80):
    """
    ...
    for _ in range(n_mc):
        ...
        per_acc += per_rs_vec(ser_vec, t_error)
    return per_acc / n_mc


def per_rs_expected_two_path(k, gamma_sd_lin, gamma_hd_lin,
                               n_sys=RS_N_SYM, n_extra=12, t_eff=RS_T_BASE+6, n_mc=80):
    """
    ...
    for _ in range(n_mc):
        ...
        per_acc += per_rs_vec(ser_vec, t_eff)
    return per_acc / n_mc
...
def carq_hop(snr_db, max_retries=MAX_RETRIES):
    ...
    t_round = HOP_DIST/SOUND_SPEED*2 + 1.0 + 0.3  # rtt + margin + helper overhead
    return e_r * t_round, e_c, q
...
def charq_hop(snr_db, max_retries=MAX_RETRIES):
    ...
    t_round = HOP_DIST/SOUND_SPEED*2 + (NX*RS_BITS_PER_SYM)/BIT_RATE + HOP_DIST/SOUND_SPEED + 1.0
    # gto = rtt + helper_tx_time + prop + 1.0
    return e_r * t_round, e_c, q
...
def cacharq_hop(snr_db, max_retries=MAX_RETRIES):
    ...
    if cpkt == 3:
        t_round = t_p + (n_x*RS_BITS_PER_SYM)/BIT_RATE + 2*t_p + 0.15  # Grace: helper TX + ACK back, no source TX
    else:
        t_round = HOP_DIST/SOUND_SPEED*2 + (n_x*RS_BITS_PER_SYM)/BIT_RATE + HOP_DIST/SOUND_SPEED + 1.0
        # gto: rtt + helper_tx + prop + 1.0 (matches SimPy)
    return e_r * t_round, e_c, q
...
if __name__ == "__main__":
    ...
    plot_results(SNR_WIDE, r_wide, suffix="v2_full", out_dir="output")
