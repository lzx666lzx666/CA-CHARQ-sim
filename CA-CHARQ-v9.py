...
def mc_run(snr_db, protocol, sim_time, n_runs, k=2.0):
    delays, ovhds, tputs, drops, colls = [], [], [], [], []
...
if __name__ == "__main__":
    ...
    for k_label, k_val in K_FACTORS:
        print(f"\n{'='*65}")
        print(f" CA-CHARQ v9 — {k_label} — SINR碰撞 + RTS + Grace + 全缓冲")
        print(f" SNR: -5~15dB | TARGET_MI={TARGET_MI} | skip_CA={CA_HELPER_SKIP_RATIO} skip_CARQ={CARQ_SKIP_RATIO}")
        print(f"{'='*65}")
        ...
        ax1.set_title(f"v9 Delay ({k_label})")
        ...
        ax2.set_title(f"v9 Overhead ({k_label})")

        plt.tight_layout()
        plt.savefig(f"v9_Delay_Overhead_{k_label}.png", dpi=200, bbox_inches='tight')
        print(f"\n[OK] v9_Delay_Overhead_{k_label}.png")
        ...
        ax3.set_title(f"v9 Throughput ({k_label})")

        plt.tight_layout()
        plt.savefig(f"v9_Throughput_{k_label}.png", dpi=200, bbox_inches='tight')
        print(f"[OK] v9_Throughput_{k_label}.png")
        plt.close('all')

        # 文本汇总
        print(f"\n{'='*65}")
        print(f"v9 Delay Summary ({k_label})")
        ...
        print(f"\n{'='*65}")
        print(f"v9 Overhead Summary ({k_label})")
        for i, s in enumerate(SNR_LIST):
            parts = []
            for proto in PROTOCOLS:
