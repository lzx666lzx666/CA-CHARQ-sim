#!/usr/bin/env python3
"""
CA-CHARQ v6 (MAX) — Multi-Chain Concurrent Cooperative HARQ with MAC
=====================================================================
v6 fixes: B1 contend() coordinate computation, B2 C-ARQ/C-HARQ chain_id,
B3 helper c_pkt from PER, D4-6 competition realism (backoff jitter + ACK cancel).

Scenario: 3 fan-shaped chains, sink-convergent, 5 hops × 600m each.
Helpers: 6 randomly placed in overlapping region (extreme scarcity).
Controls: S&W/C-ARQ/C-HARQ with 15 pre-assigned helpers (5 per chain).
MAC: half-duplex + receiver capture (single signal at a time).
"""
...
CHARQ_FEC_EXTRA  = 12
MAX_PARITY_QPSK  = 54
MIN_CAPTURE_SNR_LINEAR = 0.05  # signals below ~-13dB don't capture receiver
CHAIN_Y_OFFSETS = [-600, 0, 600]  # v6: used for coordinate computation in contend()
T_BACKOFF_JITTER  = 0.08   # v6: random backoff jitter for competition realism
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"  CA-CHARQ v6 MAX — Multi-Chain + MAC | {N_RUNS} MC × {len(SNR_LIST)} SNR × {len(PROTOCOLS)} proto")
    ...
    ax1.set_title("CA-CHARQ v6 MAX — 3-Chain with MAC")
    ...
    plt.tight_layout()
    plt.savefig("output/max-v6/max-v6_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/max-v6/max-v6_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/max-v6/max-v6_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/max-v6/max-v6_EE.png")
    plt.close('all')

    print("=" * 60)
    print("CA-CHARQ v6 MAX with MAC Done.")
