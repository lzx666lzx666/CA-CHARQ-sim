...
# 8. 仿真执行
# ==========================================
def run_sim(snr_db, protocol, sim_time, seed=0, n_helpers=None):
    ...
    if n_helpers is not None:
        N_HELPERS_PER_HOP = saved_helpers
    return result
...
# 9. 蒙特卡洛
# ==========================================
def mc_run(snr_db, protocol, sim_time, n_runs, n_helpers=None):
    delays, ovhds, tputs, drops, ees = [], [], [], [], []
    succs, data_txs, nack_txs = [], [], []

    for run_i in range(n_runs):
        s = abs(42 + run_i * 7919 + int(snr_db * 3571) + (1 << 20)) % (2**31 - 1)
        r = run_sim(snr_db, protocol, sim_time, seed=s, n_helpers=n_helpers)
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print(f" Helper count sweep: {H_LIST}, SNR={FIXED_SNR}dB, SimTime={SIM_TIME}s, {N_RUNS} MC runs")
    print(f" Channel: Thorp absorption (f={FREQ_KHZ}kHz), Rician K=2")
    print("=" * 65)

    for proto in PROTOCOLS:
        print(f"\n{'='*50}\n  Protocol: {proto}\n{'='*50}")
        for n in H_LIST:
            r = mc_run(FIXED_SNR, proto, SIM_TIME, N_RUNS, n_helpers=n)
            ...
            print(f"  N_helpers={n:1d} | D={r['delay_mean']:8.1f}±{r['delay_ci95']:5.1f}s | "
    ...
    for proto in PROTOCOLS:
        ...
        if mask.any():
            ...
            ax1.plot(xm, y[mask], MARKERS[proto]+'-',
                     ...
                     markeredgewidth=0.8)
    ax1.set_xlabel("Number of Helpers per Hop")
    ...
    for proto in PROTOCOLS:
        ...
        if mask.any():
            ...
            ax2.plot(xm, y[mask], MARKERS[proto]+'-',
                     ...
                     markerfacecolor='white', markeredgecolor=COLORS[proto],
                     markeredgewidth=0.8)
    ax2.set_xlabel("Number of Helpers per Hop")
    ...
    plt.tight_layout()
    plt.savefig("v8_helpers_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] v8_helpers_Delay_Overhead.png")
    plt.close('all')
    ...
    for proto in PROTOCOLS:
        ...
        if mask.any():
            ...
            ax_ee.plot(xm, y[mask], MARKERS[proto]+'-',
                       ...
                       markerfacecolor='white', markeredgecolor=COLORS[proto],
                       markeredgewidth=0.8)
    ax_ee.set_xlabel("Number of Helpers per Hop")
    ...
    plt.tight_layout()
    plt.savefig("v8_helpers_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] v8_helpers_EE.png")
    plt.close('all')
