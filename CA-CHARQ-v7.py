...
RICIAN_K = 0.0                # Rayleigh 衰落（K=0），匹配 S-DCC 文献信道严酷度
HOP_DIST = 600.0
NUM_HOPS = 5
...
# CA-CHARQ 主动 helper（S-DCC 风格）：helper 解码后不等 NACK 直接发
CA_HELPER_SEND_RATIO = 1.5   # helper MI/TARGET > 此值 → 目的端大概率已成功，不主动发
...
class UnderwaterNode:
            ...
            for retry_i in range(MAX_HOP_RETRYS):
                ...
                if ack_ev in result:
                    ...
                    if msg['type'] == 'ACK':
                        ...
                        # CA-CHARQ: 主动 helper 已发，源端直接 IR 重传
    ...
    def handle(self, pkt):
                ...
                if pkt.rv_level == 0 and pkt.fec_idx <= 0:
                    ...
                    if mi >= TARGET_MI:
                        if self.protocol in (PROTO_CHARQ, PROTO_CARQ):
                            ...
                            c, rv = confidence_quantize(mi)
                            if mi / TARGET_MI < CA_HELPER_SEND_RATIO:
                                self.soft_buffer[pid] = {"status": "DONE", "c_pkt": c}
                                self.env.process(self.proactive_send(pkt, rv))
        ...
        # ---- NACK 处理 ----
            ...
            if self.role == 'ROUTER' and pkt.hop_rx == self.node_id:
                ...
                # CARQ: NACK 触发退避竞争（CA-CHARQ 已主动发送，此处不执行）
    ...
    # ---------- 主动 helper (S-DCC 风格)：解码后不等 NACK 直接竞争发送 ----------
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig("v7_Delay_Overhead.png", dpi=150, bbox_inches='tight')
    print("\n[OK] v7_Delay_Overhead.png")
    ...
    plt.tight_layout()
    plt.savefig("v7_DropRate_Throughput.png", dpi=150, bbox_inches='tight')
    print("[OK] v7_DropRate_Throughput.png")
    plt.close('all')
