...
class UnderwaterNode:
    ...
    def handle(self, pkt):
        ...
        if pkt.pkt_type == PKT_DATA:
            ...
            # --- helper overhear source RV0 ---
                ...
                if decode_ok:
                    if self.protocol == PROTO_CA:
                        per_now = self._compute_per(buf) if False else 0.5  # not used for helper cpkt
                        cpkt_h = compute_cpkt(per_now)
                        self.soft_buffer[pid] = {"status": "DECODED", "creation_time": pkt.creation_time,
                                                 "c_pkt": 0.5, "cpkt": cpkt_h}
                    else:
                        self.soft_buffer[pid] = {"status": "DECODED", "creation_time": pkt.creation_time}
                    ...
                    if isinstance(buf, dict) and buf.get("status") == "DECODED":
                        ...
                        if cpkt_v == 3:
                            ...
                            if cpkt_v <= 1 and (not self.is_low_snr_selected or self.helper_tx_cnt[pid] >= 3):
                                pass  # low-SNR cap or not selected
                            else:
                                link_lvl = self._link_level()
                                rv = RV_MATRIX.get(cpkt_v, {0:3, 1:3, 2:3}).get(link_lvl, 3)
                                self.helper_tx_cnt[pid] += 1
                                if self.scheduler:
                                    action, rv_out = self.scheduler.request(
                                        self.chain_id, pid, pkt.hop_tx, cpkt_v, rv,
                                        buf["creation_time"])
                                    if action == 'serve':
                                        yield self.env.process(self._send_data(
                                            pkt.hop_tx, pid, rv_out, buf["creation_time"],
                                            chain_id=pkt.chain_id))
                                    a2, rv2, tgt, p2, ct, pcid = self.scheduler.on_complete()
                                    while a2 == 'serve_next':
                                        yield self.env.process(self._send_data(tgt, p2, rv2, ct, chain_id=pcid))
                                        a2, rv2, tgt, p2, ct, pcid = self.scheduler.on_complete()
                                else:
                                    yield self.env.process(self._send_data(
    ...
    def contend(self, pkt):
        ...
        if cancel_ev not in result:
            ...
            if self.scheduler:
                action, rv_out = self.scheduler.request(
                    self.chain_id, pid, pkt.hop_tx, pkt.cpkt, rv,
                    buf["creation_time"])
                if action == 'serve':
                    yield self.env.process(self._send_data(
                        pkt.hop_tx, pid, rv_out, buf["creation_time"], chain_id=pkt.chain_id))
                a2, rv2, tgt, p2, ct, pcid = self.scheduler.on_complete()
                while a2 == 'serve_next':
                    yield self.env.process(self._send_data(tgt, p2, rv2, ct, chain_id=pcid))
                    a2, rv2, tgt, p2, ct, pcid = self.scheduler.on_complete()
    ...
    def _try_decode(self, pid):
        buf = self.soft_buffer[pid]; per_val = self._compute_per(buf)
...
# ==========================================
# 6. Topology Generator
# ==========================================
def build_topology(env, protocol, stats, ch, nv, seed):
    random.seed(seed); np.random.seed(seed)
    nodes = []
    node_counter = 0
    chain_offsets = [-600, 0, 600]  # y-offsets for 3 chains
    ...
    for cid in range(NUM_CHAINS):
        ...
        for i in range(NUM_HOPS + 1):
            ...
            if i == NUM_HOPS:
                n.is_dest = True
            routers.append(n); ch.nodes.append(n); nodes.append(n)
            node_counter += 1
    ...
    if protocol != PROTO_SW_ARQ:
        ...
        if protocol == PROTO_CA:
            n_helpers = N_HELPERS_TOTAL
        else:
            n_helpers = N_HELPERS_PER_CHAIN_CTRL * NUM_CHAINS

        placed = 0
        while placed < n_helpers:
            x = random.random() * (R * NUM_HOPS)  # 0-3000m
            y = random.random() * 1800 - 900       # -900 to +900m
            # Place within at least one chain's hop zone
            placed_ok = False
            for cid in range(NUM_CHAINS):
                y_off = chain_offsets[cid]
            ...
            h = UnderwaterNode(env, 1000 + placed, x, y, 'HELPER', protocol, stats, ch, nv, chain_id=-1)
            if protocol == PROTO_CA:
                h.scheduler = HelperScheduler(env)
            # Assign all eligible links
            best_dst = None; best_dist = float('inf')
            for cid in range(NUM_CHAINS):
                y_off = chain_offsets[cid]
                for hop in range(NUM_HOPS):
                    sx, dx = hop * R, (hop + 1) * R
                    if (math.hypot(x - sx, y - y_off) <= R and
                            math.hypot(x - dx, y - y_off) <= R):
                        h.helper_for_link.append((cid * 100 + hop, cid * 100 + hop + 1))
                        dd = math.hypot(x - dx, y - y_off)
                        if dd < best_dist:
                            best_dist = dd; best_dst = dx
            if not h.helper_for_link: continue  # not in any hop zone

            h.chain_id = -1  # multi-chain helper
            if best_dst is not None:
                h.tx_power = TX_POWER_W * min(1.0, (best_dist / HOP_DIST) ** 1.5)
            if protocol in (PROTO_CARQ, PROTO_CHARQ):
                if not h.is_selected: h.is_selected = True
            elif protocol == PROTO_CA:
                h.is_low_snr_selected = True
            ch.nodes.append(h); nodes.append(h); placed += 1

    return nodes, routers if protocol != PROTO_SW_ARQ else None
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig("output/max-v1/v5_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/max-v1/v5_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/max-v1/v5_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/max-v1/v5_EE.png")
    plt.close('all')
