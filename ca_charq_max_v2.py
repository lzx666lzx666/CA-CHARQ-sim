...
# ==========================================
NUM_CHAINS       = 3
N_HELPERS_TOTAL  = 15  # equal to control total (3 chains × 5) for fair comparison
            ...
            if self.role == 'ROUTER' and pkt.hop_rx == self.node_id:
                ...
                if link_match in self.helper_for_link:
                    ...
                    if isinstance(buf, dict) and buf.get("status") == "DECODED":
                        cpkt_v = pkt.cpkt
                        if cpkt_v == 3:
                            # Grace only: competition + scheduler for best helper
                            self.env.process(self.contend(pkt))
                        else:
                            # Cpkt=0/1/2: direct fast response (no scheduler bottleneck)
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig("output/max-v2/max-v2_Delay_Overhead.png", dpi=200, bbox_inches='tight')
    print("\n[OK] output/max-v2/max-v2_Delay_Overhead.png")
    plt.close('all')
    ...
    plt.tight_layout()
    plt.savefig("output/max-v2/v5_EE.png", dpi=200, bbox_inches='tight')
    print("[OK] output/max-v2/v5_EE.png")
    plt.close('all')
